...
@Component({
  selector: 'ceg-modal-edit-quotes',
  templateUrl: './modal-edit-quotes.component.html',
  styleUrls: ['./modal-edit-quotes.component.css']
})
export class ModalEditQuotesComponent implements OnInit {
  ...
  ngOnInit() {
    this.quoteForm = this.formBuilder.group({
      date: [this.quote.date, Validators.required],
      employee: [this.quote.employee.id, Validators.required],
      notes: [this.quote.notes],
      procedure: [this.quote.procedure.id, Validators.required]
    });
  }
  ...
  public onQuotesSubmit(form: any) {
    ...
    quoteRequest.picture_after = this.base64textStringAfter ? this.base64textStringAfter : this.quote.picture_after;
    quoteRequest.picture_before = this.base64textStringBefore ? this.base64textStringBefore : this.quote.picture_before;
    quoteRequest.procedure_id = form.procedure;
    quoteRequest.id = this.quote.id;
    quoteRequest.client_id = this.clientId;
    this.spinner = true;
    this.rest.putQuotes(quoteRequest, this.quote.id).subscribe((quotesResponse) => {
      this.getClients();
    }, (errorQuotes) => {
